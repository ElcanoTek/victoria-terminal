...
- `~/Victoria/.config/victoria/crush.json` – Crush AI agent configuration generated by the configurator at startup. This file is created from the bundled template and includes only the MCP servers for which you have provided credentials.
- `~/Victoria` – Input datasets, analysis exports, logs, and generated manifests (including `VICTORIA.md`). Treat these files as ephemeral outputs unless you copy them elsewhere.
