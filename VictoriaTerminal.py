...
def preflight_crush(
    ...
    with _Progress(
        ...
        if _which(tool.command) is None:
            _err(
                f"Missing '{tool.command}' command-line tool. Please run the Victoria Configurator first."
            )
            _sys_exit(1)
...
def launch_crush(
    ...
    try:
        ...
        _err(
            f"'{tool.command}' command not found in PATH. Please run the Victoria Configurator."
        )
        _sys_exit(1)
...
def main(
    ...
    if not SETUP_SENTINEL.exists():
        _err(
            "First-time setup has not been completed. Please run the Victoria Configurator first."
        )
        sys.exit(1)
